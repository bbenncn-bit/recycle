...
export default function BalanceOptimization() {
  const [data, setData] = useState<BalanceOptimizationData | null>(null);
  ...
  const [selectedEnergyType, setSelectedEnergyType] = useState('电力');
  ...
  // 模拟数据（基于图示样式）
  const mockData: BalanceOptimizationData = {
    timeInterval: 'day',
    ...
    facilityEquipmentData: {
      equipment: ['空压机组', '空调机组', '送风机', '排风机', '水泵', '照明系统', '其他设备'],
      consumption: [12000, 8000, 6000, 4000, 3000, 2000, 1000]
    },
    processOperationData: {
      process: ['下料', '粗加工', '氯化', '喷漆', '总装配', '包装与标识', '精加工', '零部件装配', '氧化', '热处理', '检验'],
  ...
  return (
    <div className="min-h-screen bg-blue-50 dark:bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        {/* 页面标题 */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-2">能效运行参数</h1>
          <p className="text-gray-600 dark:text-gray-300">数据更新时间: 2025-06-11 12:30</p>
        </div>

        {/* KPI 卡片 */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
          {displayData.kpiCards.map((card) => (
            <div key={card.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
              <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-4">{card.title}</h3>
              <div className="space-y-2">
                <div>
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">
                    {card.todayValue}
                    <span className="text-sm font-normal text-gray-500 dark:text-gray-400 ml-1">{card.todayUnit}</span>
                  </div>
                ...
                <div>
                  <div className="text-lg font-semibold text-gray-700 dark:text-gray-300">
                    {card.yearValue}
                    <span className="text-sm font-normal text-gray-500 dark:text-gray-400 ml-1">{card.yearUnit}</span>
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">本年度累计</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
          <div className="flex flex-wrap items-center gap-6">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-700 dark:text-gray-300">时间段</span>
              <input
                type="date"
                value={startDate}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
              <div className="flex space-x-2 mb-4">
                <select className="px-3 py-1 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                  <option>工序</option>
                </select>
                <select className="px-3 py-1 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                  <option>能源类型</option>
