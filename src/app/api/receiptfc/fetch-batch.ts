...
export async function getReceiptfcDataBatch({ page = 1, limit = 10, offset }: BatchParams = {}) {
  try {
    // 计算实际的偏移量
    ...
    const [viewExists] = await pool.query(`
      ...
      AND table_name = 'v_receiptfc_with_thumbnails'
    `);
    ...
    const [latestDateResult] = await pool.query(`
      ...
      FROM ${hasOptimizedView ? 'v_receiptfc_with_thumbnails' : 'receiptfc'}
    ...
    if (hasOptimizedView) {
      ...
      [rows] = await pool.query(`
        ...
        FROM v_receiptfc_with_thumbnails 
    ...
    console.error('❌ 分批获取receiptfc数据失败:', error);
    throw error;
  }
}
